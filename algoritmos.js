...
//8. realizar un algoritmo que solicite a estudiante su nombre, la materia y 8 calificaciones entre uno y 10 determinar con esta informacion si el estudiante aproba o reprbo teniendo en cuenta que se aprueba con 6.2 en adelante
function notas(){
    let nombre;
    let materia;
    let nota1;
    ...
    materia= parseInt(prompt("ingrese la materia  a calificar: "))
    ...
    nota7 = parseInt(prompt("ingrese la sectima nota: "))
    nota8 = parseInt(prompt("ingrese la octava nota: "))

    notafinal = nota1 + nota2 + nota3 + nota4 + nota5 + nota6 + nota7 + nota8;
    promedio = notafinal / 8

    if (promedio> 6.2){
        console.log("Paso la materia con: " + promedio)
        alert(" Paso la materia con: " + promedio)
    }

    else {(promedio<6.2)
        alert("No paso la materioa con: " + promedio)
    }


}

//9 determinar si un numero ingresado por el estudiante es par o inpar
function par(){
    let v1

    v1= parseInt(prompt("ingrese el numero: "))

    if(v1 % 2 === 0){
        console.log("el numero " + v1 + " es par")
        alert("el numero " + v1 + " es par")
    }
    else{
        alert("El numero " + v1 +" es impar")
    }
...
function calcularGanancia() {
    const inversionInicial = parseFloat(prompt("Ingrese la cantidad inicial de la inversión:"));
    const anios = parseInt(prompt("Ingrese el número de años de la inversión:"));

    if (isNaN(inversionInicial) || isNaN(anios) || inversionInicial <= 0 || anios <= 0) {
        alert("Por favor, ingresa valores válidos.");
        return;
    }

    const interesMensual = 0.007; 
    const meses = anios * 12; 
    let montoFinal = inversionInicial;
    ...
    alert(`Después de ${anios} años, tendrás $${montoFinal.toFixed(2)}`);
}
...
function mostrarRango() {
    const inicio = parseInt(prompt("Ingrese el número inicial del rango:"));
    const fin = parseInt(prompt("Ingrese el número final del rango:"));

    if (isNaN(inicio) || isNaN(fin) || inicio >= fin) {
        alert("Por favor, ingresa valores válidos. El inicio debe ser menor que el final.");
        return;
    }
    ...
    alert(`Los números entre ${inicio} y ${fin} son:\n${numerosEnRango}`);
}
